...
import { countByOrigin, estimateGameFps } from "@/engine/fps-estimate";
...
export function Builder({ catalog, prices, perfIndexes, fpsGroups }: BuilderProps) {
  ...
  // S40: FPS'e göre SIRALANMAZ. FPS'e göre dizmek kullanıcıyı "en yüksek
  // sayıyı gör" yönünde koşullandırır; oysa kullanıcı belirli bir oyunu
  // arıyor. Alfabetik, Türkçe sıralamayla.
  const fpsSorted = [...fpsRows].sort((a, b) => a.game_name.localeCompare(b.game_name, "tr"));
  const fpsCounts = countByOrigin(fpsRows);
  // Bütün satırlar aynı ayardaysa etiket bir kez başta yazılır; farklıysa
  // satır satır. Bugün tek ayar var, ama ikinci ayar geldiğinde satırın
  // hangi ayarda ölçüldüğü kaybolmamalı.
  const fpsAyarlar = new Set(fpsRows.map((row) => row.setting_label));
  const fpsTekAyar = fpsAyarlar.size === 1 ? [...fpsAyarlar][0] : null;
  // Ekran kartı seçili ama hiç satır yok: o çipin ölçümü hiç toplanmamış.
  const fpsKapsamDisi = gpuChipId !== undefined && fpsRows.length === 0;
        ...
        <div>
          ...
          {gpuChipId === undefined ? (
            ...
            <div className="flex flex-col gap-3 text-sm">
              {/* S41: tek skor ile bu liste farklı şeyler ölçüyor. Çelişki
                  gerçek ve gizlenmiyor. Test sisteminin işlemcisi ölçüm
                  satırlarımızda kayıtlı DEĞİL (cpu_part_id boş), o yüzden
                  hangi işlemci olduğu yazılmıyor — yazılsaydı kaynağı
                  olmayan bir iddia olurdu. */}
              <p className="rounded border border-amber-500/40 bg-amber-500/5 p-3 text-xs">
                Bu değerler, işlemcinin sınırlamadığı bir test sisteminde ölçülmüştür.
                Sizin işlemciniz bazı oyunlarda bu sayının altında kalmasına yol
                açabilir. Yukarıdaki sistem indeksi işlemciyi hesaba katar, bu liste
                katmaz — ikisi farklı şeyler ölçüyor.
              </p>

              {fpsTekAyar && (
                <p className="text-xs opacity-60">
                  Ayar: <span className="font-medium">{fpsTekAyar}</span>. Şu an tek
                  ayarda ölçüm var; başka çözünürlük ve preset için veri toplanmadı.
                </p>
              )}
              ...
              <ul className="flex flex-col divide-y rounded border">
                {fpsSorted.map((row) => (
                  <li key={row.game_id} className="flex items-baseline gap-3 p-2">
                    ...
                    {/* S39: ölçülmüş ve türetilmiş AYRILIR. Kullanıcı sayının
                        nereden geldiğini görmeli — bu sitenin tüm duruşu. */}
                    {row.origin === "measured" ? (
                      <span
                    ...
                    {!fpsTekAyar && (
                      <span className="text-xs opacity-50">{row.setting_label}</span>
                    )}
                  </li>
                ))}
              </ul>
              ...
              <p className="text-xs opacity-50">
                {fpsCounts.measured} oyunda sayı doğrudan ölçüm; {fpsCounts.derived}{" "}
                oyunda bu kartın ölçümü yok ve indeksinden hesaplandı. Hesaplananın
                ölçülen hata payı: ortalama %{FPS_MARGIN.meanPercent}, tahminlerin
                %90&apos;ı %{FPS_MARGIN.p90Percent} altında, en kötü %{FPS_MARGIN.maxPercent}.{" "}
                {FPS_MARGIN.method} ({FPS_MARGIN.measuredAt})
              </p>
            </div>
          )}
        </div>
