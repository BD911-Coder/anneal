...
### S48 — Wikipedia'dan gelen değer nereye yazılacak? ✅ CEVAPLANDI (2026-08-22)

**Cevap: A seçeneği — alan başına kaynak tablosu.** `spec_field_sources`
(`part_id`, `field_name` birincil anahtar) kuruldu; kaynak, güven, lisans,
makale ve revizyon numarası satırın değil **alanın** yanında duruyor.

Mevcut 2457 dolu alan `manufacturer` damgasıyla geri dolduruldu — damga
değişmedi, yalnızca alan seviyesine indi. Ardından içe aktarma gerçekten
çalıştırıldı: **50 alan yazıldı**, üretici değeri olan **96 alana
dokunulmadı**.

Atıf alanı takip ediyor: CC BY-SA kredisi yalnızca Wikipedia'dan gelen bir
değerin gösterildiği yerde çıkıyor. → `docs/KARARLAR.md` K170
